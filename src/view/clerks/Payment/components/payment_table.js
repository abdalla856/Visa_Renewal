  ...
  auth.users.users.map((user,index)=>{
    auth.app.applications.map(app=>{
      if(user.id === app.creator && app.payment === true){
        rows.push(createData(app.type,'Payment',user.name,user.id,app.payment_upload))
        console.log(app.payment_upload)
      }
    })
  })
        ...
        <TableBody>
          {rows.map((row) => (
            ...
            >
              ...
              <Link to={`/payment_check/${row.id}`} onClick={e =>(!row.uploaded)? e.preventDefault() :null}>
