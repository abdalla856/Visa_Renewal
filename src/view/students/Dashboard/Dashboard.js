...
const Dashboard = () => {
 ...
 const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getAppByUserId(auth.token,auth.userId))

  },[dispatch])
  const data = useSelector((state) =>state.apps)
  console.log(data)
 //  setData(data.application)
  auth.app = data.application
  console.log(auth) 
  return (
    <React.Fragment>
      <nav>
      ...
      <div className="container_dash">
        ...
        >
          ...
          <SwiperSlide>
            <img
              className="img_in_slider"
              src={'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSSlcTF5yI_DxeZ0kES3hgJaHCsr7Fm0e7GBw&usqp=CAU'
